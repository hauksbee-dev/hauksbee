...
`run <board> --emit-netlist --json` prints the extracted connectivity and
exits, so another tool can ask hauksbee what is connected to what instead of
parsing the board file. It replaces the report rather than joining it:

```json
{"board": "two_resistors",
 "nets": [{"name": "Net-(R1-Pad2)",
           "pins": [{"ref": "R1", "pin": "2", "pin_name": "~"},
                    {"ref": "R2", "pin": "1", "pin_name": "~"}]}]}
```

`pin` is the pad or pin number as printed, `pin_name` the schematic pin name
(empty on a layout). Nets sort by name and pins by refdes then number, so two
runs are byte-identical; unconnected pins appear in no net. This is
extraction, not binding: it answers on a board with no models, with the
do-not-populate policy already applied. Without `--json` the same content is
one tab-separated `net`/`refdes`/`pin`/`pin-name` line per connected pin.
