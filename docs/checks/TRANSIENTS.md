...
(`crates/hauksbee-cosim/src/peripherals/load.rs`) that owns an `Isource`
...
two internal nodes (`crates/hauksbee-checks/src/decoupling.rs`):
...
(`crates/hauksbee-bind/src/power_supply.rs`):
...
This is the headless, scriptable counterpart to watching a net in the live web
viewer, and pairs with the scenario runner above: probe the rail while a `[[scenario]]` load
