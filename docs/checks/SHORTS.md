...
- **Zone-versus-pad overlaps are suppressed as a class, and the run says so.**
  ...
  turn the second into the first. `None` yields its own note saying so.
  `DrcReport::suppression_note()` renders the disclosure, which all three DRC
  surfaces print ahead of any clean claim (the text report as `NOT CHECKED:`, the
  plain report as a heads-up, and JSON as `suppression_note`). That is a smaller
  set than the "four batch report surfaces" the co-sim docs count: hauksbee-ci
  runs no DRC at all, and the web front door does not read the field. Boards
  where nothing was suppressed get no note. To audit the suppressed class, run
  KiCad's own DRC.
