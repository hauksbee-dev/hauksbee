...
## See a board live (the web viewer)

`hauksbee run <board> --serve` (or the `hauksbee serve` subcommand) serves the
live web viewer; bare `hauksbee run <board>` prints the report instead. The
...
trusted. Add `--report` for the static verdict, or `--serve` for the live
viewer.
