...
A published release would use versioned tags such as `:slim-0.1.0-beta.3` and
`:full-0.1.0-beta.3`. The beta attaches none, so pin the immutable digest of
...
- uses: hauksbee-dev/hauksbee/integrations/github-action@REPLACE_WITH_RELEASE_COMMIT_SHA
  with:
    ...
    hauksbee-version: v0.1.0-beta.3
    spec: ci/watchy.toml
    use-image: true
