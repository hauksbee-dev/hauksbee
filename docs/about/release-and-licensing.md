...
- **The CLI does real work**: `doctor`, `run --check` (bind report, DRC, lint,
  ...
  (LF-only, no CRLF, no unescaped backslash paths), and `to-code` / `from-code` /
...
- **`serve` carries the full web surface**, driven from a real host browser
  ...
  backend returns the discovery guidance, and dependency auto-install refuses
  on Windows with manual instructions).
- **Discovery knows the Windows conventions**: `Renode.exe` in the
  ...
  plus `%ProgramFiles%\Renode` and `%LOCALAPPDATA%\Programs\Renode`, and
...
- The test suite itself, console/TTY behavior, real NTFS semantics,
...
- A real installer run: `scripts/get-hauksbee.ps1` exists, follows the
  ...
  clean, and a full download-verify-extract-install dry run against a mock
  release server, including the checksum-mismatch refusal). It has not run
  on a real Windows box, and there are no published Windows release assets
  for it to fetch yet.
...
> it should. The real work starts at making `cargo test --workspace
