...
| ADC inject (`set_analog_in`) | yes | per-platform `AdcChannelMap` (Monitor feed command or result-word write), and only where a descriptor declares one: `renode:stm32f072` inputs 0..7 and `renode:rp2040` inputs 0..3 are live-proven. STM32F103/F4/nRF52/FE310 model no ADC peripheral in their stock platforms, so injections there are DROPPED and surfaced on every report surface that runs the external backend (run text, `--plain`, `--json`, and hauksbee-ci), never silently. The synchronous web front door refuses external-emulator runs before these scheduler signals exist and points to live/CLI co-sim instead. See "ADC / bus coverage by platform" | yes, RAM-mailbox count slots (firmware contract) |
...
synchronous web front door does not run a
...
synchronous web report refuses
...
it per descriptor before a run happens. The simavr web run can report a reboot, while the synchronous
...
synchronous web report does not run
...
| Coverage caveat | default text | `--plain` | `--json` | hauksbee-ci | web front door |
|---|---|---|---|---|---|
| dropped ADC injections | yes | yes | yes | yes | not run (external backend) |
| unexercised buses | yes | yes | yes | yes | not run (external backend) |
| watchdog limitation | yes | yes | yes | yes | not run (external backend) |
| watchdog reboots | yes | yes | yes | yes | yes |
| timing limitation | yes | yes | yes | yes | not run (external backend) |
| per-core timing coverage | yes | yes | yes (field) | yes | yes (field) |
| strict timing replay refusal | yes | yes | yes (field) | yes (refuses timing claims) | yes (field + refusal) |
| fallback-qualified windows | yes | yes | yes (field) | yes (field/qualification) | yes (field) |
| short pulses | yes | yes | yes | yes | yes |
| driver contentions | yes | yes | yes | no | yes |
| drive conflicts | no | yes | yes | no | yes |
| heuristic SPI framing | yes | yes | yes | assertion detail | yes (field) |

The synchronous web front door carries every disclosure its
...
**One enumeration behind all five columns.**
...
- **Crystal-clocked boards.** A crystal/resonator is bound high-impedance
  ...
  (`crates/hauksbee-bind/src/binder.rs`, `is_crystal_like`), because a
...
- **ADC injection needs a per-platform map on Renode.** Renode's ADC
  ...
  reports), naming the channel, MCU, and net, never a stderr-only whisper. The synchronous web report refuses this backend
