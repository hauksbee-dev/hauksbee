...
extension that stays core Rust, deliberately. Zero-dispatch stamping is a
performance pillar, so devices are enum variants, not trait objects. A
...
new variant slips past them, and the worst failure is a plausible wrong
waveform.
...
Also, when the device owns a branch-current unknown (anything that fixes a
...
`examples()`* panics until your example ships. Once it ships, your variant
enters the serde round-trip automatically
...
> cannot pass that way (the comment above the assert in `lib.rs` says exactly
> this). Matches force *handling*. The inventory forces *testing*.
...
extraction and tear-engine island building both route through it. Both
matches are exhaustive **with no `_` arm on purpose**, so your variant fails
to compile until you wire it in. For the VCVS all four terminals remap:
...
current ever crossed it). A terminal declared sense whose stamp actually
leaks current would break torn solves silently. So the code enforces the
claim mechanically:
...
asserts (a) conduction plus sense covers every terminal exactly once and (b)
...
Whatever you stamp, `reserve_pattern` must pre-touch every slot. The
sparsity pattern is frozen before solving, and an `add_at` outside it panics
(the good outcome) or corrupts a neighbor (the bad one). The VCVS reserves
its branch incidence plus the `(branch, cp)`/`(branch, cn)` control columns.
...
waveform, the worst of the six hazard classes.
...
1. **The enforcement suite** (free once `examples()` is extended):
   ...
   A green run shows: `test decompose::conduction::tests::declared_sense_rows_receive_no_current ... ok`
...
3. **The ngspice deck gate**: a `.cir` deck plus a companion
   ...
   A green run shows a per-deck pass line with the worst relative error
   printed. (The harness needs `ngspice` on PATH. Without it the comparison
   cannot run, so install it rather than trusting the other two layers
   alone.)
