...
//!   1. the `GALVANI_RENODE` environment variable (full path to the binary),
//!   2. a `renode` on `PATH`,
...
pub fn find_renode() -> Result<PathBuf> {
    if let Some(p) = std::env::var_os("GALVANI_RENODE") {
        let p = PathBuf::from(p);
        if p.exists() {
            return Ok(p);
        }
        bail!("GALVANI_RENODE is set to '{}' but it does not exist", p.display());
    }
    ...
    bail!(
        "Renode not found. Install it (https://renode.io) and either put `renode` \
         on PATH, set GALVANI_RENODE to the binary, or extract the portable build \
         to ~/renode-portable."
    )
}
