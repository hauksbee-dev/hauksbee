...
//! boards. Per `docs/evidence/CORPUS.md`, each was chased to the XML before being
...
fn famous_eagle_boards_have_no_true_shorts() {
    ...
    assert!(
        ...
        "the famous Eagle boards shipped and must be short-clean; if one really \
         shows a short, chase it to the XML before believing it (docs/evidence/CORPUS.md). \
         Offenders:\n  {}",
        offenders.join("\n  ")
    );
}
