...
pub fn from_gerber_dir(dir: &Path) -> Result<GerberExtraction, ExtractError> {
    ...
    for d in &drills {
        ...
        let is_gerber = drill_is_gerber_format(
            &head,
            d.extension().and_then(|s| s.to_str()),
        );
        if is_gerber {
...
mod drill_sniff_tests {
    ...
    fn excellon_with_gerber_in_banner_is_not_misrouted() {
        ...
        // no RS-274X structural markers. The old `contains("Gerber")` catch-all
        // sent it to the gerber parser and dropped every hole.
    ...
    fn genuine_gerber_drill_film_still_detected() {
        ...
        assert!(drill_is_gerber_format("%FSLAX34Y34*%\n%MOMM*%\n", Some("gbr")));
