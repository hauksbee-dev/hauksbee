...
/// self-transmutes, same-type casts and undocumented `unsafe fn`s for a C
/// header's typedefs, none of which anyone can act on: the file is regenerated
/// from simavr's headers on every build. Silencing them here keeps the rest of
/// the workspace under a clean `-D warnings` gate instead of drowning it in
/// several hundred warnings about code we do not write.
#[cfg(feature = "avr")]
#[allow(
