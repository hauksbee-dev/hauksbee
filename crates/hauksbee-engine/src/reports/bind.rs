//! The bind report and the `--report` surface that prints it.
//!
//! [`BindReport`] is one row per board component recording what model it
//! resolved to and what it became in the simulation: an analog or behavioral
//! device, a digital or MCU block, a power rail, a deliberately-skipped part, or
//! an unresolved open circuit. [`BindOutcome`] is that per-component verdict and
//! the report aggregates them for the resolve-rate stats.
//!
...
//! passives and `--verbose` restores every component row. [`emit`] prints the
//! table in one [`OutputMode`], augmented by [`BindSummary::render_banner`].
//! This is a description of the board, not a pass/fail check, so `--strict` does
//! not apply; `--plain` adds a one-line bottom verdict.
