...
//! timing coverage). A user watching a live co-sim saw no caveat and could
//! conclude the board was fully modelled. This regression covers the two AVR
//! classes the real production path can emit; external-backend-only classes are
//! explicitly unavailable on the synchronous web path.
//!
