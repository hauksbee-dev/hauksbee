//! Transient scenario plumbing for galvani-ci.
//!
//! A `[[scenario]]` block attaches a dynamic [load profile](galvani_models::LoadProfile)
...
impl InlineProfile {
    /// Convert to the model-layer [`galvani_models::LoadProfile`].
    pub fn to_profile(&self) -> galvani_models::LoadProfile {
        galvani_models::LoadProfile {
            id: self.id.clone(),
