//! Known-fault validation: run galvani against open-hardware board revisions
//! that fixed a real, *documented* electrical fault between two public releases.
//! The revision history is ground truth: "galvani flags rev N for exactly the
...
//! `docs/KNOWN_FAULTS_VALIDATION.md`. The point of encoding them as tests is
...
fn i2c_findings(r: &galvani_extract::NetLintReport) -> usize {
...
fn i2c_medium_plus(r: &galvani_extract::NetLintReport) -> usize {
    use galvani_extract::Severity;
...
// translated bus genuinely needs its own resistors. galvani flags it MEDIUM.
// Fixed 1.2.1: R504/R505 (3k3) added; galvani goes clean. This is the strongest
...
// the pulls WERE present; here we assert galvani stays clean on it, so the
...
//
// galvani does NOT flag these, and the validation doc explains why this is the
