...
"#;
    ...
    let lax = galvani_extract::run_drc(&doc, Some(0.2));
    assert!(lax.is_clean() && lax.findings.is_empty(), "0.2 mm rule: clean");
    let strict = galvani_extract::run_drc(&doc, Some(0.5));
