...
//! `GALVANI_REQUIRE_CORPUS=1` is set (then a missing corpus is a failure).
...
fn output_contention_is_silent_on_known_good_corpus() {
    let Some(root) = corpus() else {
        assert!(
            std::env::var("GALVANI_REQUIRE_CORPUS").is_err(),
            "GALVANI_REQUIRE_CORPUS set but board-corpus is absent"
        );
