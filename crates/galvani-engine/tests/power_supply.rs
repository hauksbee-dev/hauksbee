...
use galvani_engine::power_supply::{Chemistry, PowerSupply, UsbSpec};
...
fn engine_for(rload_ohms: &str) -> GalvaniEngine {
    let text = load_board(rload_ohms);
    GalvaniEngine::from_board_file(&text, None, "/boards/load.kicad_pcb").expect("build engine")
}

/// Run `n` chunks of `dt` and return the settled +5V rail voltage and the
/// supply's last rail current.
fn settle(engine: &mut GalvaniEngine, dt: f64, n: usize) -> (f64, f64) {
