...
//! a galvani LM75 slave to the co-sim, sweeps its temperature across the
...
use galvani_engine::{GalvaniEngine, I2cBus, Lm75};
...
fn run_at_temp(temp_c: f64, ms: u32) -> f64 {
    ...
    let fw = firmware();
    let mut engine =
        GalvaniEngine::from_bound(bound, Some(&fw), "/ci").expect("build engine");
