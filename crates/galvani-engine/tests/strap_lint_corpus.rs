...
//!
//! Corpus-gated (skipped when board-corpus is absent), with GALVANI_REQUIRE_CORPUS=1
//! turning absence into a hard fail so the calibration cannot vacuously green-out.

use std::path::PathBuf;

use galvani_engine::checks::straps::strap_lint;
use galvani_extract::{ExtractedBoard, LintCheck, NetLintReport, Severity};
...
fn famous_root() -> Option<PathBuf> {
    ...
    if std::env::var("GALVANI_REQUIRE_CORPUS").is_ok() {
        panic!("GALVANI_REQUIRE_CORPUS set but board-corpus/famous is missing: {}", p.display());
    }
...
fn strap_findings(r: &NetLintReport) -> Vec<&galvani_extract::LintFinding> {
