...
fn focused(path: &PathBuf, keep: &[&str]) -> Option<galvani_engine::BoundBoard> {
    if !path.exists() {
...
// from the net. galvani now MODELS the internal pull (previously invisible), so
...
fn npm1300_shphld_feeds_gpio_on_120_and_is_clean_on_121() {
    ...
    // (the pull is internal to the part, present regardless). galvani now sees
