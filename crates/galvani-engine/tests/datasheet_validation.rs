...
//! DC operating point with the real galvani solver, and assert physical sanity:
//!
...
use galvani_ir::{BjtModel, Circuit, Device, DiodeModel, NodeId, Polarity, SourceKind};
use galvani_models::schema::{ComponentKind, DbFile, ModelEntry};
...
fn dc_try(circuit: &Circuit) -> Result<galvani_solve::Waveforms, String> {
...
fn dc(circuit: &Circuit) -> galvani_solve::Waveforms {
    dc_try(circuit).expect("DC operating point converges")
}

fn final_v(wf: &galvani_solve::Waveforms, circuit: &Circuit, net: &str) -> f64 {
...
fn source_current(wf: &galvani_solve::Waveforms, name: &str) -> f64 {
    wf.branch_currents
        .iter()
        .find(|(n, _)| n == name)
