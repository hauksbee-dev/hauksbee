...
//! `forge-codegen` and galvani's co-simulation. The flow:
//!
//! ```text
...
//!                                              │
//!                              galvani_extract::ExtractedBoard::from_kicad_pcb
//!                                              │
//!                                          bind_board
//!                                              │
...
use crate::engine::GalvaniEngine;
...
pub fn check_board_text(board_text: &str, opts: &CheckOptions) -> anyhow::Result<CheckReport> {
    ...
    let mut engine = GalvaniEngine::from_bound(bound, None, "/boards/check")?;
    if opts.destructive {
