...
use galvani_ir::{BjtModel, Circuit, Device, NodeId, Polarity, SourceKind};
use galvani_solve::{Integration, Partitioning, SolverOptions, StepControl, Transient};
...
fn synapse_array_partitioned_vs_monolithic() {
    ...
    let ngspice = "/opt/homebrew/bin/ngspice";
    if std::path::Path::new(ngspice).exists() {
        let path = std::env::temp_dir().join("galvani_synapse.cir");
        ...
        println!("  galvani partitioned / ngspice = {:.3}", part_t.as_secs_f64() / ng.as_secs_f64());
...
fn accuracy_partitioned_vs_monolithic() {
    ...
    for (name, spec, tstop, dt) in cases {
        let c = if *spec == "GEN_LADDER20" {
            ...
            galvani_ir::SpiceLoader::load(spec).unwrap()
        };
