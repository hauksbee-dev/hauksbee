...
fn diode_eval(model: &DiodeModel, vd: f64, t_c: f64, temp_on: bool) -> (f64, f64) {
    ...
    let nvt = model.n * galvani_ir_thermal(t_c, temp_on);
    if vd >= -3.0 * nvt {
        let e = (vd / nvt).exp();
        let id = is * (e - 1.0);
        let gd = is * e / nvt;
...
fn galvani_ir_thermal(t_c: f64, temp_on: bool) -> f64 {
    let t = if temp_on { t_c } else { 27.0 };
    galvani_ir::thermal_voltage_c(t)
}
...
fn stamp_diode(
    ...
    let is = if temp_on { model.is_at(t_c) } else { model.is };
    let vt = galvani_ir_thermal(t_c, temp_on);
    let nvt = model.n * vt;
    let vc = vcrit(is, nvt);
...
fn stamp_mosfet(
    ...
    let sign = model.polarity.sign();
    let vt = galvani_ir_thermal(ctx.opts.model_temp(), ctx.opts.effects.temperature);
    let beta = model.beta();
