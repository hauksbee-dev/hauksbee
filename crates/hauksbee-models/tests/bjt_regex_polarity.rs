...
fn bc3xx_pnp_entry_excludes_npn_bc317_318_319() {
    ...
    for v in ["BC307", "BC308", "BC309", "BC320", "BC327", "BC328", "BC557"] {
        assert_eq!(resolved_id(&lib, v).as_deref(), Some("bc307"), "{v} should be the PNP entry");
    }
...
fn bc2xx_npn_entry_is_not_over_broad() {
    ...
    for v in ["BC237", "BC238", "BC239", "BC547", "BC548"] {
        assert_eq!(resolved_id(&lib, v).as_deref(), Some("bc237"), "{v} should be the NPN entry");
    }
    // The old range caught BC240-259; those should no longer bind here.
    for v in ["BC240", "BC250", "BC259"] {
        assert_ne!(resolved_id(&lib, v).as_deref(), Some("bc237"), "{v} should not match bc237");
    }
}
