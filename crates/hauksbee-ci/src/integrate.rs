...
pub fn try_github_workflow_yaml() -> anyhow::Result<String> {
    ...
    github_workflow_yaml_for(source_commit)
}

fn github_workflow_yaml_for(source_commit: Option<&str>) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "# Hardware CI: run hauksbee-ci on every change that could break the board.\n\
         ...
         \x20         hauksbee-token: {}\n\
         \x20         hauksbee-ref: {}\n\
         \x20         # A clean development commit may not have a release tag.\n\
         \x20         # Build the exact pinned source instead of guessing one.\n\
...
mod tests {
    ...
    fn generated_workflow_refuses_missing_or_malformed_source_identity() {
        for source in [None, Some("0000")] {
            let err = github_workflow_yaml_for(source).unwrap_err().to_string();
