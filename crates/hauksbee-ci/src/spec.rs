...
impl Assertion {
    fn validate(&self) -> Result<(), SpecError> {
        match self.kind.as_str() {
            ...
            other => {
                return Err(SpecError::Invalid(format!(
                    "unknown assertion kind '{other}' (expected voltage|uart|toggle|no_faults|max_current|peripheral|rail_window|protection_trip|boot-coverage|phase_margin|ac_gain)"
                )));
            }
        }
        Ok(())
