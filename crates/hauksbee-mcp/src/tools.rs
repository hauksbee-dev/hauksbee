...
pub fn definitions() -> Value {
    json!([
        ...
        {
            "name": "run_script",
            "description": "Code mode: run a JavaScript program server-side against the hauksbee API and return the composed result in ONE call, instead of many tool round-trips. The sandbox (embedded QuickJS) exposes exactly one capability, the global `hauksbee` object: analyzeBoard(path, firmwarePath?), runChecks(path, specToml, firmwarePath?), listCapabilities(), boardToCode(path). Each returns the same JSON object the corresponding tool returns. console.log(...) is captured into the response. No filesystem, no network, no imports, no other globals beyond the JS builtins. The script runs as a function body: use `return` for its result, which must be JSON-serializable. HONESTY CONTRACT inside the sandbox: a refusal is THROWN as a structured error object with .status === \"invalid_for_analysis\" so a script cannot accidentally treat it as data; catch it if you want to handle it. Tool input errors are thrown as {error: message}. Response: {result, logs}; an uncaught throw comes back as an error with {thrown, logs}. Scripts are killed after 120 seconds.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "source": {
