"""Tests for the pcbnew-free plugin core. Run with:
    ...
    python3 -m pytest integrations/kicad-plugin/test_hauksbee_ci_core.py
    # or, with no pytest:
...
<testsuites name="hauksbee-ci" tests="2" failures="1" time="0.10">
...
def test_build_command():
    cmd = core.build_command("/bin/hauksbee-ci", "ci/power-up.toml", "/tmp/r.xml")
    assert cmd == ["/bin/hauksbee-ci", "run", "ci/power-up.toml", "--junit", "/tmp/r.xml"]
...
def test_run_ci_with_mocked_runner_failure(tmp_path=None):
    ...
    run = core.run_ci("spec.toml", binary="/bin/hauksbee-ci", runner=fake_runner)
...
def test_find_binary_finds_prebuilt_bundle():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        os.makedirs(bindir)
        fake = os.path.join(bindir, "hauksbee-ci")
...
def test_ensure_binary_returns_found_without_building():
    ...
    try:
        core.find_binary = lambda explicit=None: "/usr/local/bin/hauksbee-ci"
        got = core.ensure_binary(build=True, runner=_runner)
        assert got == "/usr/local/bin/hauksbee-ci"
