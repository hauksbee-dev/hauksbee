...
import { buildToml, tomlToBuilder } from '../src/lib/check-spec'
...
describe('shared constraint editor', () => {
  test('uses the Checks preflight for a voltage constraint', () => {
    ...
    expect(issues.flatMap(issue => issue.fields)).toEqual(['net', 'min', 'max'])
  ...
  test('supports strict over-voltage rail windows with the shared polarity control', () => {
    ...
    expect(constraintIssues(spike).flatMap(issue => issue.fields)).toEqual(['spike_above'])
    spike.spike_above = '3.6'
    expect(constraintIssues(spike).flatMap(issue => issue.fields)).toEqual(['spike_for_max_ms', 'settle_within_ms'])
