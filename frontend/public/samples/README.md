...
Mostly copies of repo fixtures, with two deliberate exceptions. A sample is the
...
`boot_gate.kicad_pcb` is one case: the fixture carries two deliberate GND/+5V
...
`watchy.kicad_pcb` is the other deliberate exception. KiCad keeps this design's
0.150 mm default clearance in the sibling `.kicad_pro`; a one-file browser
sample cannot see that sibling. The browser copy therefore adds only
`(min_clearance 0.15)` to the board-local setup. Its copper, parts and placement
remain byte-for-byte the upstream board, while browser and checkout DRC now use
the same real design rule instead of manufacturing 0.200 mm warnings.
...
| `watchy.kicad_pcb` + `watchy.LICENSE` | `crates/hauksbee-ci/examples/boards/` plus its project clearance | A real shipped product (sqfmi Watchy): what a report looks like on a non-trivial board. |
