...
fn build_kicad_pcb(b: PcbBuilder) -> List {
    ...
    children.push(Sexpr::list(
        "version",
        vec![Sexpr::atom(b.version.to_string())],
    ));
    // (generator "pcbnew")
    children.push(Sexpr::list(
        "generator",
        vec![Sexpr::Token(Token::string("pcbnew"))],
    ));

    // (general (thickness 1.6))
    let general = Sexpr::list(
        "general",
        vec![Sexpr::list("thickness", vec![Sexpr::atom("1.6")])],
    );
    ...
    children.push(Sexpr::list(
        "paper",
        vec![Sexpr::Token(Token::string("A4"))],
    ));
    ...
    let setup = Sexpr::list(
        "setup",
        vec![Sexpr::list("pad_to_mask_clearance", vec![Sexpr::atom("0")])],
    );
    children.push(setup);
    ...
    if !has_net0 {
        children.push(Sexpr::list(
            "net",
            vec![Sexpr::atom("0"), Sexpr::Token(Token::string(""))],
        ));
    }
    for net in &b.nets {
        children.push(Sexpr::list(
    ...
    Sexpr::list(
        ...
        vec![
            Sexpr::list(
                "start",
                vec![
                    Sexpr::atom(fmt_f64(gl.start.0)),
                    Sexpr::atom(fmt_f64(gl.start.1)),
                ],
            ),
            Sexpr::list(
                "end",
                vec![
                    Sexpr::atom(fmt_f64(gl.end.0)),
                    Sexpr::atom(fmt_f64(gl.end.1)),
                ],
            ),
...
fn build_footprint(fp: FootprintBuilder) -> List {
    ...
    children.push(Sexpr::list(
        "layer",
        vec![Sexpr::Token(Token::string(&fp.layer))],
    ));
    children.push(Sexpr::list(
        "at",
        vec![
...
fn build_segment_node(s: SegSpec) -> Sexpr {
    let mut args = vec![
        Sexpr::list(
            "start",
            vec![
                Sexpr::atom(fmt_f64(s.start.0)),
                Sexpr::atom(fmt_f64(s.start.1)),
            ],
        ),
        Sexpr::list(
            "end",
            vec![Sexpr::atom(fmt_f64(s.end.0)), Sexpr::atom(fmt_f64(s.end.1))],
        ),
...
fn build_via_node(v: ViaSpec) -> Sexpr {
    ...
    let mut args = vec![
        Sexpr::list(
            "at",
            vec![Sexpr::atom(fmt_f64(v.at.0)), Sexpr::atom(fmt_f64(v.at.1))],
        ),
