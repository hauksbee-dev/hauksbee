...
#
# galvani's core (extract, bind, solve, the static checks, board-as-code, the
...
# (Set GALVANI_DOCTOR_STRICT=1 to exit non-zero if a REQUIRED tool is missing.)
set -euo pipefail
...
log "galvani environment doctor"
printf '\n'

[ "$QUIET" -eq 1 ] || log "Required (the toolchain that builds galvani)"
report required cargo "build + install the galvani / galvani-ci binaries (https://rustup.rs)"
...
if [ "$missing" -eq 0 ]; then
  ...
  log "${present} present, ${missing} optional/absent. Core galvani (static checks, board-as-code, built-in AVR co-sim) works now."
fi

if [ "$required_missing" -gt 0 ] && [ "${GALVANI_DOCTOR_STRICT:-0}" = "1" ]; then
  die "a required tool is missing (GALVANI_DOCTOR_STRICT=1)"
fi
exit 0
