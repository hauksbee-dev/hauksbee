...
Stated at the precision it deserves, since "zero false positives" is a strong claim: re-running the lint across the public corpus, exactly one medium-or-high finding survives on those boards, and it is real. The Olimex ESP32-EVB routes a free-running 50 MHz oscillator onto the ESP32's GPIO0 boot-strap net, where a clock edge at the reset latch can drop the part into download mode; it fires on every revision from B to L. Every other board is silent above Low, where the Lows are the I2C-breaks-out-to-a-header convention.

That silence is enforced, not merely observed. Checks carry corpus silence gates that go red if the check ever fires on a known-good board: the placeholder-value gate alone sweeps every board file it finds (129 on our fetch) across all four extraction paths and requires zero medium-or-high findings, and there are equivalents for output contention, the SI checks and the MCU resource conflicts. Each skips when no corpus is present and hard-fails under `HAUKSBEE_REQUIRE_CORPUS=1`, so a gate cannot report green on zero coverage. The honest caveat: several of the older corpus guards address boards by a layout `fetch-corpus.sh` does not produce, so they do not yet run outside the maintainers' tree. Full detail, per board and per check, including the false positive this calibration had to kill: [`docs/evidence/FAMOUS_SWEEP.md`](docs/evidence/FAMOUS_SWEEP.md).
