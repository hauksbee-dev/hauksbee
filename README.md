...
Here is that claim at the precision it deserves, because "zero false positives" is a stronger thing than the evidence supports. Re-running the lint across the public corpus today, the medium-and-high findings on those boards come to exactly two. One is real: the Olimex ESP32-EVB's GPIO0 boot-strap net carries a free-running 50 MHz clock, which can drop the part into download mode, and it fires on every revision from B to L. The other is a false positive: the Arduino Uno's `RESET-EN` solder jumper has no component value, correctly, and the placeholder-value check reads that as an unfinished BOM entry. Every other board in the corpus is silent above Low. So: one true positive, one false positive being fixed, and no defect the boards' own maintainers had not already found.

What holds that in place is narrower than a single gate. Individual checks have corpus silence guards that fail if the check ever fires on a known-good board (output contention, the SI checks, the MCU resource conflicts), and the lint's own suite pairs every true positive with the exact false-positive shape that once fooled it. There is no single gate asserting the whole lint stays quiet corpus-wide, and several of the existing corpus guards address boards by a layout the public fetch does not produce, so they do not currently run outside the maintainers' tree. Both gaps are recorded in the write-up rather than papered over. Full detail, per board and per check: [`docs/evidence/FAMOUS_SWEEP.md`](docs/evidence/FAMOUS_SWEEP.md).
