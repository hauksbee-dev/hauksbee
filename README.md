...
- **Olimex RP2040-PICO-PC, revisions C and D**: RP2040 PWM slice 6A double-booked between the PicoDVI pixel clock and the PWM audio. Upstream issue #1. Correctly silent on rev B's netlist, where the clock sits on a different slice.
...
- **MNT Reform motherboard 2.0 and 2.5**: no pull-up on the on-board DAC I2C bus. This one we have **not** adjudicated, and it is the honest open question in the set.

So the claim we will defend is "the lint does not invent findings on shipped hardware", not "the lint is silent on shipped hardware". Silence above Low is the common case, not the universal one, and the Lows throughout are the I2C-breaks-out-to-a-header convention.

Parts of that are enforced by corpus silence gates that go red if a check fires on a known-good board (output contention, the SI checks, the MCU resource conflicts, placeholder values), each hard-failing under `HAUKSBEE_REQUIRE_CORPUS=1` rather than skipping quietly. Two caveats we would rather state than have you discover: several of those gates address boards by a directory layout `fetch-corpus.sh` does not produce, so they do not yet run outside the maintainers' tree, and the nightly corpus workflow does not currently have a green path. Full detail, per board and per check: [`docs/evidence/FAMOUS_SWEEP.md`](docs/evidence/FAMOUS_SWEEP.md).
