...
Part of that is enforced by corpus silence gates that go red if a check fires on a known-good board, and it is worth being exact about how much. Of the four, only the placeholder-value gate currently exercises a fetched corpus (129 board files, four extraction paths, passing). The output-contention gate scans **zero** boards, because its board list is relative to a `board-corpus/famous/` layout `fetch-corpus.sh` does not produce; the SI and MCU-resource gates run partially, for the same reason. So the gates are real and their present reach is one of four. That is also why the nightly corpus workflow has no green path, and why `HAUKSBEE_REQUIRE_CORPUS=1` currently fails on board location rather than on any check firing. Full detail, per board and per check: [`docs/evidence/FAMOUS_SWEEP.md`](docs/evidence/FAMOUS_SWEEP.md).
